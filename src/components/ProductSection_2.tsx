...
export default function ProductDetailPage() {
  ...
  // ★ サイドカート表示
  const [isSideCartOpen, setSideCartOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const found = data.find((item: any) => item.slug?.toLowerCase() === slug);
  ...
  /**
   * ✅ 修正ポイント
   * - redirects.createRedirectSession は使わない
   * - Headlessの正規フロー:
   *    1) checkout.createCheckoutFromCart({ cartId })
   *    2) checkout.redirectToCheckout({ checkoutId, returnUrl, cancelUrl })
   * - Wix Hosted Checkout に遷移 → カード入力 → 購入確定 → returnUrl へ戻る
   */
  const checkout = async () => {
    try {
      ...
      // 1) カートから Checkout を作成
      const { checkout } = await myWixClient.checkout.createCheckoutFromCart({
        cartId,
      });

      // 2) Wix Hosted Checkout のURLを発行
      const { redirectSession } = await myWixClient.checkout.redirectToCheckout({
      ...
      // 3) Wixのチェックアウト画面へ遷移
      window.location.href = redirectSession.checkoutUrl;
    } catch (err) {
      console.error("チェックアウト開始に失敗:", err);
      alert("チェックアウトの開始に失敗しました。時間をおいて再度お試しください。");
    }
  };
  ...
  // ★ サイドカート：数量変更
  const changeLineItemQty = async (lineItemId: string, newQty: number) => {
    try {
      if (newQty <= 0) {
  ...
  // ★ lineItem → ItemPic を優先してサムネ決定
  const getThumbSrc = (li: any) => {
    const wixId = li?.catalogReference?.catalogItemId;
    const match = (agentProducts || []).find((p: any) => p.wixProductId === wixId);
