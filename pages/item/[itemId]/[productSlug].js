...
export default function ProductDetailPage() {
   ...
   * ✅ 修正済みチェックアウト（JS版）
   * - postFlowUrl は使用しない
   * - thankYouPageUrl を指定し、Wix に完了後の遷移を任せる
   * - redirectTo: 'CHECKOUT' で必ず Wix のチェックアウトへ遷移
   */
  const checkout = async () => {
    try {
      ...
      // 2) リダイレクト・セッション作成（thankYouPageUrl を指定）
      const { redirectSession } =
        await myWixClient.redirects.createRedirectSession({
          redirectTo: "CHECKOUT",
          checkoutId, // ecomCheckout: { checkoutId } ではなく checkoutId で渡す形
          channelType: "WEB",
          thankYouPageUrl: `${window.location.origin}/thank-you?checkoutId=${encodeURIComponent(
            checkoutId
          )}`,
        });
