...
export default function ProductDetailPage() {
  ...
  /**
   * ✅ 修正済みチェックアウト（両対応セーフティ版）
   * - まず v2 署名: intent + ecomCheckout を試す（あなたの環境の 400 "intent must not be empty" に対応）
   * - 失敗時は旧署名: redirectTo + checkoutId にフォールバック
   * - thankYouPageUrl で完了後の遷移は Wix が自動実行
   * - postFlowUrl は使わない（今回のエラー原因）
   */
  const checkout = async () => {
    try {
      // 1) 現在のカートからチェックアウトを作成
      const { checkoutId } =
      ...
      const thankUrl = `${origin}/thank-you?checkoutId=${encodeURIComponent(
        checkoutId
      )}`;

      let redirectSession;

      // 2-A) v2 署名（intent 必須）の呼び出し
      try {
        ...
        console.log("[redirects] use v2 intent=E_COM_CHECKOUT");
      } catch (e) {
        console.warn(
          "[redirects] v2 createRedirectSession 失敗。fallback へ:",
          e
        );
        // 2-B) 旧署名（redirectTo + checkoutId）
        ...
        console.log("[redirects] use legacy redirectTo=CHECKOUT");
      }

      // 3) Wix ホストのチェックアウトへ遷移（完了後は自動で thankYouPageUrl へ）
