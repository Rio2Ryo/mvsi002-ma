...
export default function ProductDetailPage() {
  ...
  /**
   * ✅ チェックアウト（v2/旧署名 両対応）
   * - v2: intent + ecomCheckout を優先
   * - 失敗時は旧署名: redirectTo + checkoutId にフォールバック
   * - thankYouPageUrl 指定で、完了後は Wix が自動リダイレクト
   * - postFlowUrl は使わない
   */
  const checkout = async () => {
    try {
      ...
      try {
        ...
        console.warn("[redirects] v2 失敗。legacy にフォールバック:", e);
      ...
      // 3) Wix のチェックアウトへ遷移（完了後は thankYouPageUrl へ）
